...
//
// Ce pont connecte les services d'intégration HA avec les modules domaine via un EventBus.
//
// **Responsabilités :**
...
export interface DomainModuleConfig {
  moduleName: string;
  entityPrefix?: string; // Préfixe pour les entity_ids HA
}
...
export class IntegrationBridge {
  ...
  private setupConfigListeners(): void {
    ...
    // Note: AppService émet 'config:save:result' avec juste les données, pas un EventBusEvent complet
    // Donc on utilise onAll et on filtre manuellement
  ...
  private setupDomainEventListeners(): void {
    ...
    this.eventBus.onTyped<EntityDiscoveryEvent>('entity:discovered', (event) => {
      this.handleEntityDiscovered(event);
    });

    // Écouter les événements de changement d'état
    this.eventBus.onTyped<EntityStateChangeEvent>('entity:state_changed', (event) => {
      this.handleEntityStateChanged(event);
    });

    // Écouter les événements de suppression d'entités
    this.eventBus.onTyped('entity:removed', (event: any) => {
      this.handleEntityRemoved(event);
    });

    // Écouter les événements de connexion/déconnexion de devices
    this.eventBus.onTyped<DeviceConnectionEvent>('device:connected', (event) => {
      this.handleDeviceConnected(event);
    });

    this.eventBus.onTyped<DeviceConnectionEvent>('device:disconnected', (event) => {
  ...
  private handleHaCommand(event: any): void {
    ...
    // @ts-ignore - emitHa attend un EventBusEvent, commandEvent est de type CommandReceivedEvent
    this.eventBus.emitHa(commandEvent);
