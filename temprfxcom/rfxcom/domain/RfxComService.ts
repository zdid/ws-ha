...
export class RfxComService {
  ...
  private configService: IAppConfigProvider<RfxComConfig>;
   ...
   * @param eventBus - Instance de IEventBus (injection depuis la couche Application)
   * @param logger - Instance du logger (injection depuis la couche Infrastructure)
   * @param configService - Instance de IAppConfigProvider<RfxComConfig> (injection depuis la couche Infrastructure)
   */
  constructor(
    eventBus: IEventBus,
    logger: Logger,
    configService: IAppConfigProvider<RfxComConfig>
  ) {
  ...
  private loadConfig(): void {
    try {
      const rfxcomConfig = this.configService.getAppConfig() as RfxComConfig | undefined;
  ...
  private async saveConfig(): Promise<void> {
    try {
      ...
      // Sauvegarder via IAppConfigProvider
      await this.configService.savePartialConfig(rfxcomConfig);
